...
function drawGraphic() {
  ...
  // console.log(graphic_data, series, columns)

  // nestedByName = d3.group(graphic_data, d => d.name)
  //
  // // get ranks for the data
  // nestedByTimePeriod = d3.group(graphic_data, d => d.timePeriod)
  // ranks = {};
  // columns.forEach((item, i) => {
  //   let ranks = d3.rank(nestedByTimePeriod.get(item), d => +d.value)
  //   nestedByTimePeriod.get(item).forEach((unit, j) => {
  //     unit["rank"]=ranks[j]
  //   });
  // });


  // data = d3.group(graphic_data,d=>d.timePeriod,d=>d.name)
  // data.forEach((column)=>{
  //   column.forEach((value,key,map) => {
  //     value[0]["rank"]=d3.rank(Array.from(map.values()),d=>d[0].value)[[...map.keys()].indexOf(key)]
  //   });
  //
  // })
...
data.forEach((item) => {
  item[2]=d3.rank(item[1],d=>+d.value)
  item[1].forEach((d, i) => {
    d.rank=item[2][i]
  });
});

// flatten but just keep the objects like graphic data
